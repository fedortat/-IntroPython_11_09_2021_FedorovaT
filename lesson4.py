...
# 1 способ:
# result = f"{my_symbol}\n" * my_symbol_count
# result = result.strip() # метод strip убирает все непечатные символы
# print(result, "---------")
...
# my_list = my_list + list(new_str) # выведет то, что нужно, но айди списка поменяется
# 2 способ:
my_list += list(new_str)  # НЕ ТО ЖЕ САМОЕ, ЧТО my_list = my_list + list(new_str) - айди списка не меняется
print(id(my_list), my_list)
